...
- Stored artifact policy is two-tier. Strict authorities are normalized `BusinessStateV2`, `SiteIntentV2`, immutable `SourceSnapshotV1`, `AssetRevisionV1`, `FormDefinitionV2`, `SitePublicBuildInputV1`, `SiteWorkspaceRevisionV1`, `SiteBuildArtifactV1`, public `SiteVersionV4`, `TrustedRuntimeSeriesV1`, and `TrustedRuntimePatchV1`. Public rendering, owner truth, publish gates, form handling, runtime security, and auditability depend on these shapes.
...
- Agent traces, screenshots, critiques, transient build logs, and prompt/debug artifacts are regenerable intermediates. They may be added or reshaped without historical backfills, but any intermediate that affects a candidate must carry producer, model, skill, input-hash, timestamp, and stale/regeneration provenance where practical.
