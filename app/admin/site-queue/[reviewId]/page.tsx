  ...
  return <main className="admin-page">
    ...
    <div className="admin-grid">
      ...
      <aside className="panel"><h2>Decision</h2>
        ...
        {run ? <><dl className="detail-list"><dt>Run</dt><dd>{run.status} · {run.stage}</dd><dt>Model route</dt><dd>{run.apiProvider ?? run.executionDriver} · {run.modelId ?? run.externalProvenance?.clientReportedModelId ?? "unverified"}</dd><dt>Failure</dt><dd>{run.failureCategory ?? "unknown"} · {run.failureCode ?? "unclassified"}</dd><dt>Model cost</dt><dd>{run.usage.kind === "model_reported" && run.usage.costSource !== "unavailable" ? `$${run.usage.costUsd.toFixed(4)} · ${run.usage.costSource.replaceAll("_", " ")}` : "Unavailable"}</dd><dt>Diagnostic</dt><dd>{run.failureReason ?? "None"}</dd></dl><Link className="button secondary" href={`/admin/runs/${run.id}`}>Run diagnostics</Link></> : null}
