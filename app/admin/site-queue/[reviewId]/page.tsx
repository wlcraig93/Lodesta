...
export default async function OperatorReviewPage({ params }: { params: Promise<{ reviewId: string }> }) {
  ...
  const candidateIntegrity = version ? await deriveSiteCandidateIntegrity({ versionId: version.id, repository: sitePlatformRepository }) : undefined;
  const sourceCoverage = version ? await sitePlatformRepository.getSiteVersionSourceCoverage(version.id) : undefined;
  ...
  return <main className="admin-page">
    <header className="admin-header"><div><span className="badge">Operator review</span><h1>{site.slug}</h1><p>{item?.reason.replaceAll("_", " ") ?? `Candidate version ${version?.number}`}</p></div>
      <div className="button-row"><Link className="button secondary" href="/admin/site-queue">Back to queue</Link><Link className="button secondary" href={`/workspace/${site.slug}/editor`}>Open workspace</Link></div></header>
    <div className="admin-grid">
      ...
      <aside className="panel"><h2>Decision</h2>
        ...
        {run ? <><dl className="detail-list"><dt>Run</dt><dd>{run.status} · {run.stage}</dd><dt>Model route</dt><dd>{run.apiProvider} · {run.modelId}</dd><dt>Failure</dt><dd>{run.failureCategory ?? "unknown"} · {run.failureCode ?? "unclassified"}</dd><dt>Model cost</dt><dd>{run.usage.costSource !== "unavailable" ? `$${run.usage.costUsd.toFixed(4)} · ${run.usage.costSource.replaceAll("_", " ")}` : "Unavailable"}</dd><dt>Diagnostic</dt><dd>{run.failureReason ?? "None"}</dd></dl><Link className="button secondary" href={`/admin/runs/${run.id}`}>Run diagnostics</Link></> : null}
        {candidateIntegrity ? <><h3>Candidate integrity</h3><ul>{candidateIntegrity.issues.map((issue) => <li key={`${issue.code}:${issue.referenceId ?? "site"}`}>{issue.message}</li>)}{candidateIntegrity.status === "current" ? <li>The retained candidate is current and technically intact.</li> : null}</ul></> : null}
        <OperatorQueueActions queueItem={item} />
      </aside>
    </div>
    {sourceCoverage ? <SourceCoveragePanel report={sourceCoverage} /> : null}
