...
export default async function AdminSitePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="admin-page">
    <AdminPageHeader eyebrow="Managed site" title={state?.identity.name ?? slug} description={`${site.id} · ${state?.vertical.id ?? "unclassified"}`} actions={<div className="button-row"><Link className="button secondary" href="/admin/sites">Sites</Link><Link className="button primary" href={`/editor/${slug}`}>Workspace</Link>{published ? <Link className="button secondary" href={`/sites/${slug}`}>Live site</Link> : null}</div>} />
    <section className="metric-row"><Metric label="Site" value={site.status} /><Metric label="State revision" value={state?.revision ?? "-"} /><Metric label="Intent revision" value={intent?.revision ?? "-"} /><Metric label="Open review" value={openQueue.length} /></section>
    <div className="admin-grid">
      ...
      <aside className="panel"><h2>Business authority</h2><dl className="detail-list"><dt>Phone</dt><dd>{state?.contacts.phone ?? "Not recorded"}</dd><dt>Email</dt><dd>{state?.contacts.email ?? "Not recorded"}</dd><dt>Offerings</dt><dd>{state?.offerings.length ?? 0}</dd><dt>Confirmed proof</dt><dd>{state?.proof.filter((item) => item.status === "confirmed").length ?? 0}</dd><dt>Eligible assets</dt><dd>{state?.assets.filter((item) => item.activeForFutureBuilds).length ?? 0}</dd><dt>Candidate versions</dt><dd>{candidates.length}</dd></dl></aside>
    </div>
