...
export default async function AdminSitePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="admin-page">
    ...
    {site.status !== "paused" ? <section className="panel"><div className="section-heading-row"><div><h2>Claim link</h2><p className="muted">For a prospect project you built (or an unowned one). A claim link gives the signed-in account that redeems it control of this project. It does not prove business ownership. Links are single-use and expire after 14 days.</p></div></div><ClaimLinkPanel siteId={site.id} /></section> : null}
