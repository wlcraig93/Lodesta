...
export default async function AdminSitePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!site) notFound();
  const [state, intent, versions, runs, changes, queue, claims] = await Promise.all([
    ...
    platformOperationsRepository.listClaims(site.id)
  ]);
  ...
  const ownership = deriveSiteOwnership(claims);
  return <main className="admin-page">
    ...
    <section className="panel"><div className="section-heading-row"><div><h2>Site tools</h2><p className="muted">Open the canonical owner surface for this site.</p></div></div><div className="button-row"><Link className="button secondary" href={`/workspace/${slug}`}>Home</Link><Link className="button secondary" href={`/workspace/${slug}/website`}>Website</Link><Link className="button secondary" href={`/workspace/${slug}/inbox`}>Inbox</Link><Link className="button secondary" href={`/workspace/${slug}/results`}>Results</Link><Link className="button secondary" href={`/workspace/${slug}/business`}>Business</Link><Link className="button secondary" href={`/workspace/${slug}/settings`}>Settings</Link><BillingPortalButton siteId={site.id} returnPath={`/admin/sites/${slug}`} disabled={!claims.find((claim) => claim.status === "claimed")?.stripeCustomerId} disabledReason="No Stripe customer is attached to this site." /></div></section>
