...
export async function GET(request: Request, { params }: { params: Promise<{ assessmentId: string }> }) {
  ...
  const eligible = new Set(record.assessment.dimensions.flatMap((dimension) => dimension.criteria)
    .flatMap((criterion) => criterion.evidence.map((item) => item.artifactKey).filter((value): value is string => Boolean(value))));
