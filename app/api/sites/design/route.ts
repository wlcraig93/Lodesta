...
const designSchema = z.object({
  ...
  sectionVariants: z.record(z.string().min(1)).optional()
});
...
export async function POST(request: Request) {
  ...
  if (!parsed.data.themePreset && !parsed.data.sectionOrder && !parsed.data.sectionVariants) {
    return NextResponse.json({ error: "Provide a themePreset, sectionOrder, or sectionVariants." }, { status: 400 });
  }
