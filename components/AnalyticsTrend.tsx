...
export function AnalyticsTrend({ points }: { points: AnalyticsTrendPoint[] }) {
  if (!points.length) {
    return <div className="workspace-empty-state"><strong>No activity in this date range</strong><p>The trend will appear after a counted page view.</p></div>;
  }
  const width = 720;
  const height = 220;
  const inset = 24;
  const max = Math.max(1, ...points.flatMap((point) => [point.pageViews, point.customerActions]));
  ...
  const path = (key: "pageViews" | "customerActions") => points.map((point, index) => {
  ...
  return (
    <div className="analytics-trend">
      <div className="analytics-trend-legend" aria-hidden="true"><span><i />Page views</span><span><i />Customer actions</span></div>
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-labelledby="analytics-trend-title analytics-trend-desc">
        <title id="analytics-trend-title">Page views and customer actions over time</title>
      ...
      <details className="analytics-trend-table">
        ...
        <div className="analytics-data-table" role="region" aria-label="Trend data" tabIndex={0}>
          <table><thead><tr><th>Date</th><th>Page views</th><th>Customer actions</th></tr></thead><tbody>
            {points.map((point) => <tr key={point.bucket}><th>{formatBucket(point.bucket)}</th><td>{point.pageViews}</td><td>{point.customerActions}</td></tr>)}
          </tbody></table>
        </div>
      </details>
    </div>
