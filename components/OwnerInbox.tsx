...
export function OwnerInbox({ siteId, slug, initialInquiries, eventsByInquiry, requestedInquiryId, timeZone, renderedAt }: {
  siteId: string;
  slug: string;
      ...
      <aside className="owner-inbox-list" aria-label="Inquiries">
        ...
        <div className="owner-inbox-items">
          {filtered.map((inquiry) => <button type="button" key={inquiry.id} className={selected?.id === inquiry.id ? "is-selected" : ""} onClick={() => selectInquiry(inquiry.id)}>
            ...
            <span className="owner-inbox-item-copy"><span><strong>{inquiry.contactName ?? "Website inquiry"}</strong><small>{formatRelative(inquiry.createdAt, renderedAt, timeZone)}</small></span><p>{inquiry.aiEnrichment?.summary ?? contactPreview(inquiry, eventsByInquiry[inquiry.id]?.[0])}</p><span><WorkspaceStatus tone={statusTone(inquiry.status)}>{humanize(inquiry.status)}</WorkspaceStatus>{inquiry.aiEnrichment?.serviceInterest ? <small>{inquiry.aiEnrichment.serviceInterest}</small> : null}</span></span>
          </button>)}
      ...
      <section className="owner-inbox-detail" aria-live="polite">
        {selected ? <>
          <header className="owner-inbox-detail-header">
            ...
            <div><span>Received {formatProductDate(selected.createdAt, true, timeZone)}</span><h2>{selected.contactName ?? "Website inquiry"}</h2><p>{[selected.contactEmail, selected.contactPhone].filter(Boolean).join(" · ") || "No contact details extracted"}</p></div>
            <WorkspaceStatus tone={statusTone(selected.status)}>{humanize(selected.status)}</WorkspaceStatus>
          </header>

          {selected.aiEnrichment ? <section className="owner-inbox-ai-summary"><div><span>Lodesta triage</span><strong>{selected.aiEnrichment.intent}</strong></div><p>{selected.aiEnrichment.summary}</p><dl><div><dt>Urgency</dt><dd>{humanize(selected.aiEnrichment.urgency)}</dd></div><div><dt>Interest</dt><dd>{selected.aiEnrichment.serviceInterest ?? "Not clear"}</dd></div><div><dt>Suggested next step</dt><dd>{selected.aiEnrichment.suggestedNextAction}</dd></div></dl></section> : null}
          ...
          <section className="workspace-panel owner-inbox-history"><div className="workspace-panel-heading"><div><span>History</span><h3>Inquiry activity</h3></div><small>{selectedEvents.length} event{selectedEvents.length === 1 ? "" : "s"}</small></div><div className="owner-inbox-timeline">{selectedEvents.map((event) => <article key={event.id}><span /><div><strong>{humanize(event.type)}</strong><p>{event.messageText ?? eventSummary(event)}</p><small>{humanize(event.actor)} · {formatProductDate(event.createdAt, true, timeZone)}</small></div></article>)}</div></section>
        </> : <div className="workspace-empty-state is-centered"><strong>No leads need attention</strong><p>New website inquiries will appear here with their original messages and contact details.</p><Link className="button secondary" href={`/workspace/${slug}/editor`}>Review website forms</Link></div>}
      </section>
    </div>
  );
}
...
function formatRelative(value: string, renderedAt: string, timeZone: string) { const delta = Date.parse(renderedAt) - Date.parse(value); if (delta < 60_000) return "Now"; if (delta < 3_600_000) return `${Math.floor(delta / 60_000)}m`; if (delta < 86_400_000) return `${Math.floor(delta / 3_600_000)}h`; return new Intl.DateTimeFormat("en", { month: "short", day: "numeric", timeZone }).format(new Date(value)); }
