...
The first fresh Kind treatment, `run_b40b340df360410da0a3cc6cbc7a297f`, succeeded through the ordinary hosted worker and hard release gate. Three separate sandbox applies succeeded across multi-minute browser-inspection gaps with no timeout, replay, or recycle. This showed continuity could succeed, but the subsequent matched run refuted the stronger claim that `keepAlive: true` had been validated.

The current product bake-off remains paused. The matched Luna/frozen-architecture Kind run `run_06d873d5f8be4a66bc81610a0e2cd441` passed the hard gate and recorded $0.09015719, but two provider recycles make both continuity and causal cost conclusions invalid. A zero-model reproduction found no duration threshold: a six-minute case failed while 7:45 and nine-minute cases passed. It also exposed that six simultaneous sessions exceeded the deployment's five-instance limit while `keepAlive` prevented idle release. The corrective configuration uses a 15-minute idle window, preserves deterministic full-source recovery for irregular host replacement, and seeds exact required portal links in initial source. Existing inputs remain unchanged pending synthetic verification and the exact matched Kind rerun.
