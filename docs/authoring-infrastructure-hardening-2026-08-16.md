...
The current product bake-off remains paused. The matched Luna/frozen-architecture Kind run `run_06d873d5f8be4a66bc81610a0e2cd441` passed the hard gate at $0.09015719, but it exposed a longer-gap lifecycle failure that the first treatment did not reach. After inspections lasting approximately 7:45 and 8:00, the next identical-source sandbox apply timed out at 60 seconds and recovered only after a recycle. The treatment is therefore infrastructure-invalid under the predeclared acceptance rule. Existing inputs remain unchanged; the next step is to make sandbox continuity survive the full inspection ceiling and rerun this exact matched arm before Surge or the broader reliability screen.
