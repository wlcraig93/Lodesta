...
The crawler retains fetched website resources and source-page associations in the immutable source mirror; it does not automatically turn a capped set of photographs into business assets. Source preparation materializes a usable canonical logo when identified. Other source images become business-scoped immutable asset revisions through ordinary `adopt_source_asset` authoring, retaining source provenance and the existing fetch, format, byte and pixel safety boundaries. The source mirror's batch-write concurrency is not a retained-photo limit. Full photo decoding at adoption is distinct from retaining a fetched source resource.

The canonical request includes bounded high-detail source and managed-asset contact sheets. Initial builds receive up to eight source candidates and eight managed assets; edits and rebases receive up to eight source candidates and two managed assets. These are preview limits, not source-retention limits or exhaustive visual inspection. Managed asset IDs are immediately usable; source resource IDs require adoption. Canonical semantic-neutral context omits old alt text and filename/page cues from the relevant previews rather than presenting them as verified image descriptions.
...
## Source-photo delivery preparation — reader-first rollout

The September 17 reader change accepts a second strict source preparation recipe, `source-photo-web` version 1, without changing the existing `logo-presentation` member or unprepared retained assets. The pre-change read-only report parsed all 259 retained asset revisions successfully; none contained the new recipe. Reader support alone does not create derivatives or change delivered bytes.

The separately reviewed producer prepares **newly adopted source photos only**, at the existing adoption boundary. It retains the exact crawl resource as evidence and selects one auto-oriented WebP delivery revision (maximum edge 2,560px, quality 90, effort 4) when orientation/size requires it or the candidate is smaller. Otherwise it retains the original bytes. Complete decode must succeed under the existing 80-million-pixel limit; decoded PNG/JPEG/WebP must match the declared MIME type, and animated input is rejected before derivative persistence. No delivery-time transformation, responsive variant service, historical rewrite, owner-upload change or generated-image change is introduced.

Prepared provenance records the raw source hash, MIME and dimensions and exact recipe configuration. Changed revision identity binds source snapshot/resource, raw hash, recipe version and prepared hash. Identical transformed bytes from different source identities must retain distinct source-bound revisions; exact repeated adoption of the same active identity reuses its existing revision. Source hosting and adoption establish provenance, not proof that the depicted subject is this business's work.

Reader release `37500b2313a0a52094951ffa2f5ee8d8207b5d9f` is deployed and independently verified across web, worker and active sandbox, with a fresh-session synthetic canary. The release's first canary timed out after losing journal visibility; its retained built-in retry passed. This is not proof that intermittent infrastructure failures are resolved. Once the first new-recipe revision is retained, **the rollback floor is that verified reader-compatible release**: e8ee and older strict readers cannot read the new revision. Compatible controller and sandbox identities are retained in its release checkpoint. No feature flag, parallel authoring implementation or artifact rewriting is needed. Producer deployment and real adoption remain separate pending evidence, not implied by reader acceptance.
