...
Deploy and verify readers before enabling this producer. Once the first new-recipe revision is retained, **the rollback floor is the verified reader-compatible release**: e8ee and older strict readers cannot read that revision. Retain that compatible web/worker deployment and sandbox identity before producer rollout. No feature flag, parallel authoring implementation or artifact rewriting is needed. The reader release and later producer deployment/verification must be recorded separately in the current status; this section is not a claim that either is live.
