...
For an approved document, verification requires the complete approved word stream in rendered content, allowing markup and the surrounding site shell. A provision-level check of the original document is insufficient for a targeted correction: the old document could still contain the one obsolete statement. Original unapproved documents use the binary completeness check (owner decision, September 24, 2026): the legal body, with the repeated site shell and boilerplate such as "last updated" and copyright lines removed, is split into its substantive provisions (lines of six or more words), and every provision must appear in the rendered page after whitespace, punctuation, case and markup normalization. The former 85% five-word-shingle retention threshold is removed. The exact source path is still required. Missing approved routes/text and missing unrelated legal routes remain hard failures. Unrelated safety, factual, destination and capability checks are unchanged.
