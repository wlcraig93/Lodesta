...
3. The candidate is deployed directly to the inactive Worker. The workflow waits up to 20 minutes for Cloudflare to report the inactive container application `ready` on the exact expected image digest, then reads the image-resident manifest through a fresh disposable sandbox session. This bounded readiness observation happens before maintenance and is not a canary retry. The full health, source-policy, compilation, backup, restore, and exact-manifest canary runs once after readiness is proven. The 20-minute bound reflects an observed healthy rollout that remained in Cloudflare's `provisioning` state beyond ten minutes.
