...
The bounded-visual rerun was `run_2ad38b8e310849e68cfe4593a23b55b3` / `version_2a53d1b26ed01b41ccb7445a0763bbc5`. It passed the hard gate at $0.12196158 in 1,330,676 ms, with 17 model requests and four inspections. The visual phase consistently fell to 81–82 seconds and complete post-recovery inspections fell to 237–244 seconds, so the inspection simplification is accepted independently.

The run is still infrastructure-invalid: its first two `/apply` attempts reached Cloudflare but were canceled at the edge after about 29.9 seconds each, before the container began the candidate build. The identical payload built in 11.8 seconds after deterministic recycle. Signed-in Cloudflare logs then identified a deployed `@cloudflare/sandbox` version mismatch (`sdk=0.12.7`, `container=0.12.3`) and undisposed returned process RPC stubs. Align the image and SDK and dispose those handles before another paid run. Do not treat the numerically lower cost than R8 as causal evidence.

The aligned-image release removed that 30-second transport signature, but a deterministic retained-source reproduction showed the container filesystem disappearing across the exact 465-second idle gap. The SDK's `getSandbox` helper applies lifecycle options asynchronously without awaiting the Durable Object RPC. The canonical Worker now awaits its 15-minute sleep configuration before any filesystem operation. This preserves the simple bounded-idle architecture and avoids heartbeats, automatic repair, or dual sandbox behavior.

The quality result also does not justify promotion. V4 preserved recipes, the seeded portal, and a custom form, and its hero/brand treatment was strong. It also retained 56 advisory warnings versus R8's zero warnings, used more repair inspections, produced a less conventional desktop form grid, and left unreachable duplicate footer source. Keep V2/R8 canonical, do not run Surge, and validate the sandbox fix with a deterministic source-heavy lifecycle test before spending on another matched site.

## August 21 portable-glyph correction

The later recipe-free, modal-contained V4 bake-off promoted V4 after it beat matched R8 on Kind, remained at least comparable on Surge, and passed the six-run reliability rule. A subsequent audit found a separate deterministic platform gap: five of the six V4 artifacts relied on unguaranteed system fallback for `↗`, `↯`, `✓`, or `✳`. One captured hero badge rendered `NO GLYPH`. The old gate verified font loading but did not prove that a bundled face covered each visible codepoint, and the nine-category visual rubric did not explicitly treat portable glyph rendering as a prerequisite.

The correction is central and forward-only. Pinned same-family symbol faces guarantee the four observed characters for every managed family, a checked coverage manifest binds exact binaries to canonical cmap ranges, and `render.missing_glyph` reports unsupported visible text during inspection and blocks finalization. Agent-authored emoji is not supported; icons use accessible inline SVG. Owner-authoritative unsupported emoji is never silently removed.

Do not change the frozen blinded scores. Portable glyph rendering becomes a technical precondition for future comparisons. V4 promotion remains correct: R8's navigation ownership failure was structural and repeated across both businesses, while this font defect is deterministic and fixed once in the platform without returning presentation ownership to the runtime.
