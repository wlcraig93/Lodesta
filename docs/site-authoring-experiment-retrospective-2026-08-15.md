...
Do not run Surge. A zero-model six-session reproduction subsequently showed one six-minute provider stop while 7:45 and nine-minute sessions passed, and exposed the deployment's five-instance cap against unbounded `keepAlive` sessions. Restore a bounded idle lifecycle above the inspection ceiling, structurally seed exact required portal destinations in initial source, verify the corrected sandbox without model spend, and then rerun this exact matched Kind arm. Do not add authoring orchestration or move presentation back into the trusted runtime.
