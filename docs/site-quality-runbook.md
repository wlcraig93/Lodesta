...
If the authorized cleanup removes workspace destinations that are still named by the earlier seven-day workspace rollback manifest, retire that overlap expectation only with `retire:workspace-rollback-after-site-v3`. The command requires the completed cleanup receipt, empty V3 authorities, the active lease, zero running work, exact destination/sidecar deletion evidence, and hash-matching retained artifact-bucket sources. It keeps those sources as recovery orphans and writes a private receipt; never hand-create the cleanup marker.
