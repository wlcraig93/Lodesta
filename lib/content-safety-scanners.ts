...
const sensitiveClaimPatterns: Array<Omit<SensitiveClaimMatch, "matchedText" | "start" | "end"> & { pattern: RegExp }> = [
  ...
  { category: "marketing", label: "best or #1 claim", severity: "warning", pattern: /(?:\bthe\s+best\b(?!\s+(?:way|time|place|fit)\b)|\bbest\s+(?:[a-z-]+\s+)?(?:prices?|control|service|services|company|contractor|provider|team|choice|option|solution|results?)\b|#\s?1\b|\bnumber\s?one\b)/i, requiredEvidence: "proof" },
  ...
  { category: "emergency", label: "emergency availability claim", severity: "warning", pattern: /\b(24\s*(?:\/|x)\s*7|same day|emergency|after hours|around[-\s]the[-\s]clock|day (?:and|or) night)\b/i, requiredEvidence: "emergency" },
  ...
  { category: "safety", label: "safe service-performance claim", severity: "warning", pattern: /(?:\b(?:safe|safer)\s+(?:[a-z-]+\s+)?(?:removal|relocation|treatment|application)\b|\b(?:remove|relocate|treat|apply|address|handle|control|resolve|manage)\b.{0,40}\bsafely\b|\bsafely\b.{0,40}\b(?:remove|relocate|treat|apply|address|handle|control|resolve|manage)\b)/i, requiredEvidence: "proof" },
  { category: "safety", label: "safer-space outcome claim", severity: "warning", pattern: /\bsafer(?:[-\s]+feeling)?\s+(?:homes?|rooms?|yards?|properties|spaces?|environments?)\b/i, requiredEvidence: "proof" },
