...
export const websiteManagerTools: Tool[] = [
  ...
  tool("inspect_site", "Build the current workspace if needed, then inspect the already-running pages in Lodesta's browser and return measured render findings as text, including every release blocker finish would report on any page (contrast, navigation, links, layout). The default result is text only: it does not attach screenshots. For an initial build, pass null for the architecture-selected starting sample; passing '/' inspects only the homepage. Pass an exact route with selector null to attach one desktop top screenshot of that route. Pass an exact route and CSS selector to attach one focused screenshot of the first matching visible element. Route null keeps the representative measured sample without pictures. Use a route or selector screenshot only when measured text leaves a concrete visual uncertainty. This never runs hard release verification.", {
