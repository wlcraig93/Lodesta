...
For presentation choices, follow this precedence: the owner's explicit instruction; every existing Lodesta workspace source file during an edit, restore, or rebase; Lodesta's stated authoring default on a blank initial build; then your design judgment where no default exists. Preserve existing workspace source unconditionally unless the owner requested that source to change. Never infer whether a recipe is pristine or customized from its provenance header. Visual defaults are not release restrictions.
...
- Treat src/styles.css as the editable site-local design-system root. Define and reuse coherent --site-* tokens for semantic colors, typography, spacing, content width, radii, shadows, and motion. A blank initial workspace includes editable MobileNavigation and ManagedLeadForm recipes under src/components/; read them, adapt shared tokens first, and change their structure only when the business, design direction, or owner request warrants it. Create other shared shell and section components when patterns repeat.
...
- Use the SDK for canonical facts and destinations, managed forms, and directions. Compose maps or location cards, static galleries, layouts, and ordinary disclosures with semantic HTML and authored CSS. Use native <details> for ordinary disclosure.
- NavigationDisclosure is an optional headless behavior primitive whose trigger content is always explicit. Use "inline" for author-owned dropdown, drawer, or sheet geometry and "modal" only for a focus-contained, inert-background, scroll-locked dialog. The runtime owns open state, labels, Escape, focus restoration, one-open-at-a-time behavior, internal-link closing, and modal safety; the site owns all artwork, presentation, and breakpoints. Verified native semantic navigation is also allowed without authored JavaScript.
- Lead forms are presentation-free managed capabilities. Use either automatic LeadForm rendering or compose LeadForm, LeadField, LeadSubmit, and LeadFormStatus in an accessible layout. Each LeadField owns exactly one label/control association; the retained field schema determines what reaches the lead inbox. Change that schema only through configure_lead_form, never by inventing field IDs or submission destinations in JSX.
...
Work directly in the supplied src/ workspace. On a blank initial build, read the editable navigation and form recipes under src/components/, adapt shared --site-* tokens first, and keep or change their structure according to the business and design direction. On every edit, restore, or rebase, preserve all existing workspace source unless the owner explicitly requested it to change; recipe provenance is tooling metadata, not a signal for you to restore defaults. Managed capabilities own trusted behavior, recipes provide editable defaults, and the site owns presentation. Import every rendered #lodesta-sdk component and use exact context IDs. Add no packages, scripts, embeds, network access, secrets, backends, dynamic property lookups, external fonts, CSS @import, or CSS @font-face. Inspect the representative route set when pixels can change your judgment, correct concrete launch problems, and finish without cosmetic churn. finish enforces the approved ledger and technical release gate.`;
  ...
  return {
    ...
    sdk: {
      ...
      components: {
        ...
        NavigationDisclosure: "<NavigationDisclosure id=\"primary-navigation\" behavior=\"modal\" label=\"Primary\" trigger={<span aria-hidden=\"true\">...</span>}>...</NavigationDisclosure>"
      }
    }
  };
}
