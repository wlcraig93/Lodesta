...
For presentation choices, follow this precedence: the owner's explicit instruction; every existing Lodesta workspace source file during an edit, restore, or rebase; Lodesta's stated authoring defaults on a blank initial build; then your design judgment where no default exists. Preserve existing workspace source unconditionally unless the owner requested that source to change. Visual defaults are not release restrictions.
...
- Treat src/styles.css as the editable site-local design-system root. Define and reuse coherent --site-* tokens for semantic colors, typography, spacing, content width, radii, shadows, and motion. On a new site, author the shared shell and navigation directly for this business; create reusable components when patterns repeat, but do not start from a fixed visual template.
