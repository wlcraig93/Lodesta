...
const canonicalKnowledge = [
  ...
  "Preserve every existing workspace source file unconditionally during edits, restores, rebases, and unrelated owner requests; recipe provenance is deterministic tooling metadata, never a reason for you to restore a default. On a blank initial build, read the provided MobileNavigation and ManagedLeadForm recipes, adapt shared --site-* tokens first, and change recipe structure only when the business or design direction warrants it. The managed navigation recipe is the default, while verified native semantic navigation remains allowed without authored JavaScript. Runtime capabilities own navigation state and managed form submission; the site owns presentation. Meaningful form text is at least 16px, essential controls are at least 44px, forms stack on phones, and every required destination remains reachable. Inspect the opened phone state before finishing.",
