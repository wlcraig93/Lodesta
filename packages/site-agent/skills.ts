...
const initialBuildKnowledge = [
  ...
  "One coherent visual system, content-led pages. Choose typography, palette, spacing, surfaces, and image treatments suited to the business. Share the header, footer, tokens, controls, and small primitives; let the customer purpose determine each page's opening and main composition. Work-led, comparison-led, urgent-help, and contact pages should not all become the same hero/card-grid/process/banner with different nouns. Genuine differences require different hierarchy or decision support, not gratuitous layout variation. The first viewport should explain the service or subject and expose useful content or the primary action without an oversized headline crowding it out. Keep collection layouts deliberate and local context readable; decorative numbers and diagrams are not proof.",
  "Responsive conversion. Recompose for phone and tablet by stacking, reordering, recropping, and simplifying where useful. Keep the page purpose before its form, real imagery legible, and important actions easy to find. Give each contact method one clear role rather than repeating adjacent versions of the same phone number. Preserve geographic qualifiers and distinguish emergency availability from ordinary hours. A control must perform its promised action; use a truthful approved link or static content when an interactive capability is unavailable.",
  ...
  "Review and finish. Use inspect_site with route: null for the architecture-selected representative set; route: '/' inspects only home. Judge the supplied pixels alongside copy and retained evidence, including opened phone navigation and the complete form. Correct errors and critical or serious accessibility failures. Use warnings about contrast, text, targets, metadata, clipping, or repeated route content to find concrete causes, repairing shared causes at their canonical declaration. Advisory IA similarity is evidence, not a score: a shared structure serving the same customer job may be appropriate. Do not remove an approved route to clear an advisory. Reinspect affected routes when changed pixels remain uncertain, not as a site tour or to force warnings to zero. Finish when the complete site has no material factual, functional, accessibility, copy, or visual defect; finish performs full release verification."
] as const;
