...
function homepageVisualSummary(summary: Record<string, unknown>, feedbackMode: "launch-floor" | "component-diagnostic" | "component-diagnostic-route-family" | "component-diagnostic-route-family-shared-first" | "component-diagnostic-route-family-quality-led") {
  ...
  const baseFeedbackGuidance = iaHeuristicOnly
    ...
    : feedbackMode === "component-diagnostic" || feedbackMode === "component-diagnostic-route-family" || feedbackMode === "component-diagnostic-route-family-shared-first" || feedbackMode === "component-diagnostic-route-family-quality-led"
      ? "Correct every error and use warnings as concrete evidence, not an order to redesign unrelated working components. For render.missing_glyph, use ordinary supported text or accessible authored inline SVG for a decorative character; never silently remove owner-authoritative text. Grouped findings include up to three exampleMessages from distinct routes or viewports; use them together to repair the named element or its nearest shared component at the canonical declaration instead of discovering the same issue one route at a time. Whole-site IA findings cover the complete emitted inventory even when the pixel sheets are representative: every live route needs exactly one H1, accidental duplicated sections must be removed, and a repeated complete-main structure needs an honest same-job justification or a split composition. Architecture already owns the approved route ledger: do not remove or redirect a route now. A blocker-free result is not permission to finish while advisory.ia_structure names substantive service, hub, or location routes or advisory.ia_repetition still names a material commercial family. Make a substantive source repair before finish: deepen the named commercial routes from mapped raw evidence, replace interchangeable middle copy, preparation lists, and closing arguments with route-specific customer guidance, and vary content-led composition when the customer jobs differ. Share site-wide shells and primitives, but split a full-page data renderer when materially different customer jobs still receive the same hierarchy, section order, and decision-support format. Changing strings, class names, tone colors, and one injected middle section while retaining an identical hero/body/callout/footer sequence does not resolve that repetition. If one function renders the complete main element for sibling routes, it remains one full-page data renderer; vary the opening hierarchy and at least one main decision-support structure or split it into distinct compositions. Contact, legal, and image-credit utility routes may legitimately stay concise. The advisory need not reach an arbitrary numerical zero, but do not call finish unchanged merely because the hard blockers are gone. Rewrite every named render.internal_provenance_copy example in natural customer language and humanize advisory.raw_data_copy examples without changing their supported meaning. Rewrite named render.vague_process_copy examples when they substitute abstract process language for a concrete action, preparation, decision, or outcome. When render.form_text, render.body_font, render.tiny_text, render.target_size, or render.mobile_narrow_split identifies a shared selector or composition, repair that canonical declaration before finish. A target-size finding on navigation, contact, call, text, email, form, or another customer action is an unresolved essential-control defect; only noninteractive decorative geometry can be dismissed as minor. When render.raster_image_upscale names a prominent thumbnail, pull a genuinely higher-resolution retained source or replace that image role with a deliberate type-led composition; CSS crop or sharpening cannot create missing pixels. Clipping examples include visible text, bounds, and layout context so trace those values before editing. Compare the supplied routes side by side before finishing: remove any invented initials, text-logo, or decorative identity panel; correct any repeated fake ordinal; remove arbitrary pseudo-charts repeated across route openings; keep baked-in image lettering clear of new copy and out of partial crops; and replace oblique primary headings with a concrete service and customer situation. Inspect the opened phone navigation and correct any slash, angle, 7-like symbol, or lone stroke so the open trigger visibly reads as close. Preserve working navigation, composition, and brand decisions unless their own finding or the supplied pixels show a material failure. Readability, contrast, broken interaction, hidden content, material route-family repetition, and meaningful mobile clipping are launch issues; minor advisory geometry with no lost content or control is not a reason for cosmetic churn. Reinspect only when the changed pixels remain materially uncertain; otherwise finish after applying the concrete copy and shared-readability repairs and let deterministic release verification check operability."
  ...
  const recurringQualityReviewGuidance = qualityLed
    ? "Also replace any large geography circle or region badge with ordinary typography and layout; remove a catch-all directory group that duplicates items already named above; replace an unsupported or oblique safer-feeling primary heading; remove a decorative ordinal beside a contact summary or isolated proof statement when it communicates no real sequence; keep a contact or estimate page's H1 and concise context before its managed form on phone and tablet; and reduce a primary heading that consumes nearly the entire first viewport before its supporting sentence and action."
    : "";
  const scopeGuidance = builtRoutes.length > inspectedRoutes.length
    ...
    : visualEvidenceRoutes.length > 0 && inspectedRoutes.length > visualEvidenceRoutes.length
      ...
      : "";
  const feedbackGuidance = [scopeGuidance, baseFeedbackGuidance, recurringQualityReviewGuidance].filter(Boolean).join(" ");
  return {
    ...rest,
