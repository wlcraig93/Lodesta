...
assert(!accountRoute.includes('kind === "setup"'), "The retired setup lifecycle remains in the account entry.");
...
assert(!accountRoute.includes("item.setupId") && accountRoute.includes("removable"), "Account removal is not site-only.");
...
assert(removeWebsiteButton.includes('method: "DELETE"') && !removeWebsiteButton.includes("/cancel"), "Website removal is not bound to the canonical site disposition endpoint.");
assert(removeWebsiteButton.includes("router.refresh()"), "Website removal does not refresh the owner inventory.");
