-- Add proposed V2 managed-site copy diffs for skill reruns after acceptance.

alter table site_artifacts
  drop constraint if exists site_artifacts_scope_check,
  add constraint site_artifacts_scope_check
    check (scope in ('candidate_selected', 'candidate_alternative', 'site_selected', 'site_alternative', 'evaluation_candidate', 'qa_evidence'));
