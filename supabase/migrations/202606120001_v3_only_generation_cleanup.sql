...
where artifact_type = 'copy_diff';

-- Stale stored versions (legacy SiteVersionV3.pages projection, pre-v3
-- renderer versions) are repaired or reported by
-- `npm run backfill:strip-pages-projection`, not deleted here. Admin
-- surfaces render a "stale schema" notice for rows the backfill cannot
-- migrate, so operators decide whether to regenerate or delete.
