-- Agent-access policy is a request-time authority. Changing it does not stale an
-- otherwise current candidate or require a regenerated immutable site artifact.
...
begin
  ...
  if not exists (
    ...
    join business_states_v3 business_state on business_state.business_id = build_input.business_id
    join site_intents_v3 site_intent on site_intent.site_id = build_input.site_id
    where build_input.id = target.public_build_input_id
      and build_input.business_state_revision = business_state.revision
      and ((build_input.input -> 'intent') - 'agentAccessPolicy' - 'revision' - 'updatedAt' - 'intentHash')
        = (site_intent.intent - 'agentAccessPolicy' - 'revision' - 'updatedAt' - 'intentHash')
