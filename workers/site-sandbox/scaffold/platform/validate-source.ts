...
import { validateWorkspaceSourcePolicy, type WorkspaceSourcePolicyFileV1 } from "./source-policy";

const inputPath = process.argv[2];
if (!inputPath) throw new Error("Source-policy input path is required.");
const files = JSON.parse(await readFile(inputPath, "utf8")) as WorkspaceSourcePolicyFileV1[];
